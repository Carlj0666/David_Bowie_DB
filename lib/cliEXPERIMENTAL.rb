...
#   def show_sorted_list_of_albums
#     records_to_sort = []
#     DB_Record.all.map do |record|

#       records_to_sort << record #EXPERIMENTAL
      
#       #puts "#{index} - #{record.name}" #ORIGINAL

#     end

#     @sorted = records_to_sort.sort_by {|date| date.release_year} #EXPERIMENTAL
#     @sorted.each.with_index(1) do |record_s, index_s| #EXPEIMENTAL
    
#     puts "#{index_s} - #{record_s.name}, #{record_s.release_year}" #EXPERIMENTAL
#   end
# end #EXPEIMENTAL
...
#     if !user_index_selection.between?(0, @sorted.length - 1) #EXPER CHANGED DB_Record.all with @sorted #EXPERIMENTAL
#       user_record_selector
    
#     else
#       current_record_instance = @sorted[user_index_selection] #EXPER CHANGED DB_Record.all with @sorted #EXPERIMENTAL
