...
".white
  ...
  def show_sorted_list_of_albums
    ...
    records_to_sort << record
    end
      @sorted = records_to_sort.sort_by {|date| date.release_year}
      @sorted.each.with_index(1) do |record_s, index_s|
      puts "#{index_s} - #{record_s.name}, #{record_s.release_year}"
    end
  end
  ...
  def user_record_selector
    ...
    if !user_index_selection.between?(0, @sorted.length - 1)
      user_record_selector
    
    else
      current_record_instance = @sorted[user_index_selection]
